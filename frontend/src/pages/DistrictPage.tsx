...
import { CheckCircle2, ShieldAlert, FileText, Loader2, ListChecks, ChevronRight, TrendingUp, AlertTriangle, Truck, Users, Activity, Shield } from 'lucide-react';
...
export default function DistrictPage() {
  ...
  const [formData, setFormData] = useState({
    district_name: 'Custom Sector',
    population: 100000,
    bpl_ratio: 50,
    vulnerable_household_ratio: 40,
    hospital_demand_index: 50,
    current_lpg_stock: 50,
    expected_refill_demand: 50,
    road_access_score: 50,
    flood_or_disruption_flag: false,
    black_market_risk_score: 10,
    diversion_risk_score: 10,
    urban_induction_shift_percent: 20,
    rural_lpg_dependency_percent: 80,
    recent_supply_drop_percent: 10,
    last_mile_feasibility_score: 50
  });
  ...
  const triggerAnalysis = async (data: any) => {
    ...
    try {
      ...
      console.error(err);
      alert("Analysis Failed. Ensure backend API is running on port 8000.");
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="lg:col-span-9 flex flex-col gap-6">
             {!result && !loading && (
                <div className="h-full min-h-[500px] border border-dashed border-cyan-500/10 rounded-xl flex flex-col items-center justify-center glass-card">
                    <div className="w-16 h-16 rounded-full border border-white/5 flex items-center justify-center mb-4 bg-white/3">
             ...
             {result && !loading && (
                <div className="flex flex-col gap-6">
                   ...
                   <div className="glass-card rounded-xl overflow-hidden glow-ring">
                      ...
                      <div className="flex flex-col md:flex-row">
                          <div className="w-full md:w-2/3 p-6 border-r border-white/5">
                             ...
                             <div className="text-slate-300 text-sm font-medium leading-relaxed bg-white/3 p-5 rounded-lg border border-white/5">
                                {result.agents.moderator_agent || result.agents.moderator}
                             </div>
                          </div>
                   ...
                   {/* Agent Cards */}
                   <div>
                       <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 mb-4 px-1 flex items-center gap-2">
                           <ShieldAlert className="w-4 h-4 text-slate-700" /> Agent Reasoning Audit Trail
                       </h3>
